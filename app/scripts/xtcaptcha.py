...
class Captcha(object):
    # 把一些常量抽取成类属性
    #字体的位置
    font_path = '%s/app/scripts/verdana.ttf' %config.basedir
    #生成几位数的验证码
    number = 4
    #生成验证码图片的宽度和高度
    size = (100,30)
    ...
    @classmethod
    def gene_text(cls, num=number):
        source = list(string.letters)
        for index in range(0,10):
            source.append(str(index))
        return ''.join(random.sample(source,num))#number是生成验证码的位数
